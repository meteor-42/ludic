// Express static server for Vite build in ./dist (ESM)
...
const useSSL = NODE_ENV === 'production' && keyPath && certPath && fs.existsSync(keyPath) && fs.existsSync(certPath);

if (!useSSL && FORCE_HTTPS && NODE_ENV === 'production') {
...
// Changed from 'out' to 'dist' for standard Vite build
const distDir = path.resolve(__dirname, 'dist');

// Проверяем существование директории dist
if (!fs.existsSync(distDir)) {
  console.warn(`Directory ${distDir} does not exist. Creating it...`);
...
const startServer = () => {
  if (NODE_ENV === 'production' && useSSL) {
    ...
    // Development or no SSL - run HTTP only
    const port = NODE_ENV === 'development' ? 3001 : HTTP_PORT;
